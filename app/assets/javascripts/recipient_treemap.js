...
var recipientTreemap;
require('../stylesheets/18f.scss');
require('../stylesheets/recipient_treemap.css.scss');
...
$(document).ready(function(){
	...
	recipientTreemap = dc.treeMap("#recipient-treemap").topBarHeight(45)
								.height(500)
								.crumbTrailX(10)
								.crumbTrailY(10)
...
function loadDataAndRender(year) {
	...
	$.get("proposals/submission_fiscal_year/" + year + ".json", function(data, status) {
		...
		var orgTypeDimension = crossfilterData.dimension(function(d) {return d.ORGANIZATION_TYPE;})
		var recipientDimension = crossfilterData.dimension(function(d) {return d.PROGRAM_NAME})

		var programGroup = awardsOrgType.group();

		var levels = [{'dimension': awardsOrgType, 'columnName': 'award_organization_type'},
					  {'dimension': orgTypeDimension, 'columnName': 'ORGANIZATION_TYPE'},
					  {'dimension': recipientDimension, 'columnName': 'RECIPIENT_NAME'}];

		recipientTreemap.levels(levels);
		...
		dc.renderAll();
		recipientTreemap.filterAll();
		recipientTreemap.redraw();
