...
var qd = require('quickdash-core'),
    ...
    foundation = require('../../../node_modules/zurb-foundation/js/foundation/foundation.js'),
...
var dataFilePath = "assets/bubble_map.csv";
var mapJsonPath = "assets/us-states.geo.json";
var measureColumn = "proposal_count";
var noDataMessage = "No data for the selected filters";

$(document).ready(function() {
// start $document.ready

    //enable any foundation components
    $(document).foundation({
    ...
    $('.exit-off-canvas').click(function(){
        $('.menu-mobile-trigger').toggleClass('open');
    });

    $('#dismiss-notification').click(function(){
        $('.partial-data-message').addClass('hide-partial-data-message');
    });


    
d3.json(mapJsonPath, function(mapJson){
  d3.csv(dataFilePath, function(data){

    gbx = crossfilter(data);
    dataConf = dataConfig.instantiate({crossfilterObject: gbx, 
                           measureColumn: measureColumn})

    function addLabelAxisX(chartToUpdate, displayText) {
        chartToUpdate.svg()
            .append("text")
    ...
    function addLabelAxisY(chartToUpdate, displayText) {
        chartToUpdate.svg()
            .append("text")
    ...
    var mapContainerHeight = document.getElementById('mapContainer').offsetHeight;

    var projectionWidth = 960; 
    var projectionHeight = 700;
    var projectionLeftOffset = (width/100)*2;
    
    var projectionScale = (width/projectionWidth) * 153;
    var projectionRatio = projectionWidth / projectionHeight;
    
    var height = width/projectionRatio;
    if (height > mapContainerHeight) height = mapContainerHeight;

    var projectionTopOffset = (height/15);
    
    if (width < 740) {
        projectionLeftOffset = (width/100) * 3;
        projectionTopOffset = (height/10);
    }
    ...
    //var projection = d3.geo.mercator().scale((width/960)*150).translate([width/2, height/2])
    var projection = d3.geo.miller()
                      .scale(projectionScale)
                      .translate([(width/2) - projectionLeftOffset, (height/2) + projectionTopOffset]);

    
    var toolTipSouth = d3.tip()
            .attr('class', 'd3-tip')
            .attr('id', 'd3-tip-s')
            .direction('s')
            .offset([10, 0])
            .html(function (d) { return "<label>" + map.label()(d) + "</label><br/>" + qd.numberFormat(d.value);});
    var toolTipMapSelector = 'div#mapContainer.dc-chart circle.bubble';

    map = df.geoBubbleOverlayChart('#mapContainer')
      .width(width)
      .height(height)
      .projection(projection)
      .setGeoJson(mapJson.features, 'state', function (d) {
        return d.id;
      })
      ...
      .renderTitle(false)
      .renderlet(function(){
        if (d3.select('#d3-tip-s').empty()) {
            d3.selectAll(toolTipMapSelector).call(toolTipSouth);
        }
        d3.selectAll(toolTipMapSelector).on('mouseover', toolTipSouth.show)
                                        .on('mouseout', toolTipSouth.hide);
      })
      .dataConfigKey('state');

    yearList = dataConf.year.group.top(Infinity).map(function(d){return d.key}).sort();
    partialDataYearList = yearList.filter(function(y) {
        return (Number(y) > Number(settings.defaults.year));
    });

    tickMarks_x = yearList;
    if (width < 650) {
        tickMarks_x = [];

        for (var i = (yearList.length % 5); i < yearList.length; i = i+5) {
            tickMarks_x.push(yearList[i]);
        };
    } else if (width < 1100) {
        tickMarks_x = [];

        for (var i = (yearList.length % 2); i < yearList.length; i = i+2) {
            tickMarks_x.push(yearList[i]);
        };
    }

    function tickMarks_y() { 
        var tickMarkY_limit = dataConf.year.group.top(1).map(function(d){return d.value});
        var tickMarks_y = [0, (tickMarkY_limit/4), tickMarkY_limit/2, ((tickMarkY_limit/4)*3), tickMarkY_limit];
        if ($(window).height() < 400) {
            tickMarks_y = [0, tickMarkY_limit];
        } else if ($(window).height() < 600) {
            tickMarks_y = [0, (tickMarkY_limit/3), ((tickMarkY_limit/3)*2), tickMarkY_limit];
        }

        return tickMarks_y;
    }

    var toolTipNorth = d3.tip()
            .attr('class', 'd3-tip')
            .attr('id', 'd3-tip-n')
            .direction(function(d) {
              if(d.data.key < 1955) {
                return 'ne'
              } else if(d.data.key > 2005) {
                return 'nw'
              } else {
                return 'n'
              }
            ...
            .html(function (d) { return "<label>" + d.data.key + "</label><br/>" + qd.numberFormat(d.data.value);});//JSON.stringify(d)
    var toolTipYearSelector = 'div#year.dc-chart rect';

    year = df.barChart('#year')
      .width(width-10)
      .height($(window).height() - height - 10)
      .elasticY(true)
      .renderTitle(false)
      .x(d3.scale.ordinal().domain(yearList))
      .xUnits(dc.units.ordinal)
      .renderlet(function(){
        if ($('#d3-tip-n').length < 1) {
            d3.selectAll(toolTipYearSelector).call(toolTipNorth);
        }
        d3.selectAll(toolTipYearSelector).on('mouseover', toolTipNorth.show)
                                     .on('mouseout', toolTipNorth.hide);
      })
      .dataConfigKey('year');
    

    year.margins().right = 2;
    year.margins().left = 50;
    year.outerPadding(0);
    year.xAxis().tickValues(tickMarks_x).tickFormat(function(v){return "'" + v.toString().slice(-2);});
    year.yAxis().tickValues(tickMarks_y).tickFormat(function(v){return qd.bigNumberFormat(v);});

      year.onClick = function(d){
        var filter = year.keyAccessor()(d);
    ...
    selectedYear = df.numberDisplay('#selected-year')
                     .formatNumber(d3.format('d'))
                     .transitionDuration(0)
                     .valueAccessor(function(d){return year.filters()[0];})
                     .dataConfigKey('year');

    totalFunding = df.numberDisplay('#total-funding-value')
                     .valueAccessor(function(d){return d;})
                     .formatNumber(qd.bigNumberFormat)
                     .dataConfigKey('totalFunding');

    var getPieWidth = document.getElementById('pieCharts').offsetWidth;
    var getPieHeight = (getPieWidth * .33) < 122 ? 122 : (getPieWidth * .33);
    var getPieRadius = (getPieWidth / 6.35) < 56 ? 56 : (getPieWidth / 6.35);
    var getPieInnerRadius = (getPieWidth / 9.368) < 38 ? 38 : (getPieWidth / 9.368);
    
    var toolTipEast = d3.tip()
            .attr('class', 'd3-tip-mouse')
            .attr('id', 'd3-tip-e')
            .positionAnchor('mouse')
            .html(function (d) {
                var key = d.data.key;
                var value = qd.numberFormat(d.data.value);
                if(key === noDataMessage) value = "";
                return "<label>" + key + "</label><br/>" + value;
            });
            //JSON.stringify(d)
    var toolTipEastSelector = 'div#pieCharts g.pie-slice path';

    var pieChartDefault = {'innerRadius' : getPieInnerRadius, 'width' : getPieWidth, 'height' : getPieHeight, 'radius': getPieRadius};
    programArea = df.pieChart('#programArea')
        .height(pieChartDefault['height'])
        .width(pieChartDefault['width'])
        .cx(pieChartDefault['radius'])
        .ordering(function(d) { return -d.value})
        .radius(pieChartDefault['radius'])
        .innerRadius(pieChartDefault['innerRadius'])
        .renderLabel(false)
        .renderTitle(false)
        .slicesCap(6)
        .emptyTitle(noDataMessage)
        .legend(dc.legend().x((pieChartDefault['radius'] * 2) + 10).y(12).itemHeight(12).gap(2))
        .renderlet(function(){
            //tooltip setup
            if (d3.select('#d3-tip-e').empty()) {
                d3.selectAll(toolTipEastSelector).call(toolTipEast);
            }
            d3.selectAll(toolTipEastSelector).on('mouseover', toolTipEast.show)
                                        .on('mouseout', toolTipEast.hide)
                                        .on('mousemove', toolTipEast.updatePosition);

            //adds labels
            if (!d3.select('#programArea-label').empty()) return;

            var agencyCatRoot = d3.select('#programArea svg g');
            agencyCatRoot
              .append('svg:text')
              .attr('class','chart-label chart-label-center')
              .attr('id','programArea-label')
              .text("Funding");
            agencyCatRoot
              .append('svg:text')
              .attr('class','chart-label chart-label-center')
          ...
          .dataConfigKey('fundingAgency');

    organizationType = df.pieChart('#organizationType')
        .height(pieChartDefault['height'])
        .width(pieChartDefault['width'])
        .cx(pieChartDefault['radius'])
        .ordering(function(d) { return -d.value})
        ...
        .slicesCap(6)
        .emptyTitle(noDataMessage)
        .legend(dc.legend().x((pieChartDefault['radius'] * 2) + 10).y(pieChartDefault['radius'] - 6).itemHeight(12).gap(2))
        .renderlet(function(){
            //tooltip setup
            if (d3.select('#d3-tip-e').empty()) {
                d3.selectAll(toolTipEastSelector).call(toolTipEast);
            ...
            //label
            if (!d3.select('#organizationType-label').empty()) return;

            var assCatRoot = d3.select('#organizationType svg g');
            assCatRoot
              .append('svg:text')
              .attr('class','chart-label chart-label-center')
              .attr('id','organizationType-label')
              .text("Assistance");
            assCatRoot
              .append('svg:text')
              .attr('class','chart-label chart-label-center')
          ...
          .dataConfigKey('organizationType');

    filterBuilder = dc.filterBuilder('#filter-builder')
      .filterSources([{chart: map, icon: 'globe', label: "State"},
        {chart: organizationType, icon: 'tag', label: "Organization Type"},
        {chart: programArea, icon: 'institution', label: "Program Area"}]);

    var audioCurrency = function(key, value) {return key + " " + qd.numberFormat(value) + " dollars";};
    var yearCurrency = function(key, value) {
        return "Year " + key.toString().substring(0, 2) + "," + key.toString().substring(2, 4) + " Amount " + qd.numberFormat(value) + " dollars";};
    var audio508Translation = dc.audioDash('#audio-bubble-list')
      .charts({"Organization Type": {chart: organizationType, formatter: audioCurrency},
              "Program Area": {chart: programArea, formatter: audioCurrency},
              "State": {chart: map, formatter: audioCurrency},
              "Year": {chart: year, formatter: yearCurrency}
    });
    ...
    var resize = function(){
        ...
        dc.events.trigger(function(){
            ...
            var getPieInnerRadius = (getPieWidth / 9.368) < 38 ? 38 : (getPieWidth / 9.368);
            var pieChartDefault = {'innerRadius' : getPieInnerRadius, 'width' : getPieWidth, 'height' : getPieHeight, 'radius': getPieRadius};
            programArea.height(pieChartDefault['height'])
                .width(pieChartDefault['width'])
                .radius(pieChartDefault['radius'])
                .cx(pieChartDefault['radius'])
                .innerRadius(pieChartDefault['innerRadius'])
            ...
            organizationType.height(pieChartDefault['height'])
                .width(pieChartDefault['width'])
                .radius(pieChartDefault['radius'])
                .cx(pieChartDefault['radius'])
                .innerRadius(pieChartDefault['innerRadius'])
            ...
            year.width(width-10);
            year.height($(window).height() - height - 10);
            year.rescale();
            year.yAxis().tickValues(tickMarks_y);
            year.xAxis().tickValues(tickMarks_x);
            
            dc.renderAll();

            if (mapContainerHeight < 500) {
                addLabelAxisX(year,'Fiscal Year');
                addLabelAxisY(year,'Constant $');
            } else {
                addLabelAxisX(year,'Fiscal Year');
                addLabelAxisY(year,'Constant Dollars');
            }

            programArea.redraw();
            organizationType.redraw();
            year.redraw();
        }, 14);
    }

    advanceYearSelection = function() {
        var currentYearPosition = yearList.indexOf(year.filters()[0]);
    ...
    //setup the HTML needed to make the year dropdown work
    for (i = 0; i < yearList.length; i++) {
        $('#dropdown_changeYear').prepend('<li><a href="javascript:void(0)">'+yearList[i]+'</a></li>')
    }

    gotoYearSelection = function() {
        var _thisTarget = $(this).text();
        $('#selected-year').click();

        dc.events.trigger(function(){
          year.filterAll();
          year.filter(_thisTarget);
          year.redrawGroup();
        });

    }
    d3.selectAll('#dropdown_changeYear a').on('click', gotoYearSelection)

    qd.wireupDataAll(dc.chartRegistry.list(), dataConf);

    year.filter(settings.defaults.year);
    ...
    var yearsPartialString = "";
    partialDataYearList.forEach(function(y) {
        yearsPartialString += y + ", ";
    });
    if(partialDataYearList.length > 0) {
        yearsPartialString = yearsPartialString.substring(0, yearsPartialString.length - 2)
        if(partialDataYearList.length > 1)
            $('.partial-data-message .text').text("The years " + yearsPartialString + " have not yet been fully reported.");
        $('.partial-data-message .text').text("The year " + yearsPartialString + " has not yet been fully reported.");
    }
    

    window.setInterval(function(){ if(yearPlayerState==='playing') advanceYearSelection();}, 750)
  });
});
